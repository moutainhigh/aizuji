package org.gz.order.common.utils;
...
import org.gz.order.common.Enum.FrontSaleState;
...
public class StateConvert {
  ...
  /**
   * 据后台租借状态获取对应的前端售卖订单状态
   * 
   * @param backRentStateCode
   * @return
   */
  public static FrontSaleState getFrontSaleStateByBackRentState(int backRentStateCode) {
    switch (backRentStateCode) {
      case 3:
        return FrontSaleState.WaitPayment;
      case 5:
        return FrontSaleState.Cancel;
      case 7:
      case 8:
        return FrontSaleState.WaitSend;
      case 9:
        return FrontSaleState.SendOut;
      case 17:
      case 19:
      case 26:
        return FrontSaleState.BuyOut;
      default:
        return null;
    }
  }
   ...
   */
  public static boolean checkBuyOrderState(int reqState, int orderState) {

    // 如果 请求更新订单状态为 5 已取消 则当前订单中的订单状态需要 为 {3 待支付 }
    if (BackRentState.Cancel.getCode() == reqState) {
      if (orderState != BackRentState.WaitPayment.getCode()) {
        return false;
      }
    ...
    // 如果 请求更新订单状态为 19 正常买断 则当前订单中的订单状态需要 为 { 9 发货中 } 签收完成则流程结束变成买断状态
    if (BackRentState.NormalBuyout.getCode() == reqState) {
      if (orderState != BackRentState.SendOut.getCode()) {
        return false;
      }
    }
    // 如果 请求更新订单状态为 33 退货中 则当前订单中的订单状态需要 为 { 7 待拣货 8 待发货 9 发货中 }
    if (BackRentState.ReturnGoodIng.getCode() == reqState) {
      if (orderState != BackRentState.SendOut.getCode()
          && orderState != BackRentState.WaitSend.getCode()
          && orderState != BackRentState.WaitPick.getCode()) {
        return false;
      }
    }
    // 如果 请求更新订单状态为 34 已退货 则当前订单中的订单状态需要 为 { 33 退货中 }
    if (BackRentState.ReturnGoodEnd.getCode() == reqState) {
      if (orderState != BackRentState.ReturnGoodIng.getCode()) {
        return false;
      }
    }
    return true;

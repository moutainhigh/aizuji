...
var auditRecord = {
	...
	initCurrAuditRecord:function(){
		var _url = "/orderCreditDetail/customerCreditList";
		//加载列表数据
		var _columns = [[
		{  
		    title: '序号',// 标题 可不加
		...
		common.tableInit().init("currAuditRecord_tab",_url,function(pageParam){
			var temp = {   // 这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
				"currPage":pageParam.pageNumber,   // 页面大小
	            "pageSize": pageParam.pageSize,  // 页码
	            "orderNo":auditRecord.orderData.rentRecord.rentRecordNo
	        };
	        return $.param(temp);
		},_columns,false);
	},
	...
	initPage:function(orderData){
		...
		this.initAuditRecord();
		this.initCurrAuditRecord();
	}	
}